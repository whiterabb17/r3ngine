...
<p align="center">
  <a href="https://github.com/whiterabb17/r3ngine-mcp" target="_blank">
    <img src="https://img.shields.io/badge/r3ngine--mcp-v1.3.0-blue.svg?logo=none" alt="r3ngine MCP v1.3.0" />
  </a>
  &nbsp;
...
The [r3ngine-mcp](https://github.com/whiterabb17/r3ngine-mcp) companion (**v1.3.0**) is the official Model Context Protocol server for this instance. Cursor, Claude Desktop, VS Code, or any MCP-capable agent can list recon data, run a **singular pipeline tool** on one asset (with host-local `tool_args` schemas), propose follow-up batches, verify OSINT staging, and run **SAFE vulnerability / attack-path enrichment** (interpret-only — no exploit craft) — with a hashed API key and no delete/edit tools, database URLs, or filesystem mounts. Install/update also syncs **portable allowlisted cyber skills** into the MCP checkout (no operator-local Claude skills folder required). Identity, keys, sessions, and the request/response audit chain live in **Settings → MCP Access**. The sidecar speaks MCP only and is an HTTP client of `/api/mcp/`. Specialist Cursor agents: **r3ngine-assessor**, **r3ngine-osint**, **r3ngine-vuln-validator**.
...
🔌&nbsp;&nbsp; **MCP Access**: official [r3ngine-mcp](https://github.com/whiterabb17/r3ngine-mcp) **v1.3.0** server so IDE agents can read recon, run singular tools with validated `tool_args`, propose follow-ups, verify OSINT staging, and SAFE-enrich vulnerabilities/attack paths through `/api/mcp/` — hashed keys, sessions, portable cyber-skill sync, and an audit chain in Settings. The UI mirrors singular runs from the Subdomains tab (**Run single tool**).
...
2. Install the companion server from this checkout ([repo](https://github.com/whiterabb17/r3ngine-mcp), [v1.3.0](https://github.com/whiterabb17/r3ngine-mcp/releases)):
