...
import {
  useEmployeeIntelStore,
  type EmployeeToolKey,
  type ToolStatus,
} from '../../../../store/employeeIntelStore';
import {
  useStartEmployeeIntel,
  useStopEmployeeIntel,
  fetchEmployeeIntelReplay,
} from '../../api';

interface EmployeeIntelModalProps {
  open: boolean;
  onClose: () => void;
  scanId: number;
  onComplete: () => void;
...
const TOOL_LABELS: Record<EmployeeToolKey, string> = {
  theharvester: 'theHarvester (LinkedIn / Google)',
  linkedint:    'LinkedIn Intelligence',
  hunter:       'Hunter.io',
};

const TOOL_ORDER: EmployeeToolKey[] = ['theharvester', 'linkedint', 'hunter'];
...
export const EmployeeIntelModal: React.FC<EmployeeIntelModalProps> = ({
  open,
  onClose,
  scanId,
  onComplete,
  ...
  const { tokens } = useThemeTokens();
  const store = useEmployeeIntelStore();
  const startMutation = useStartEmployeeIntel();
  const stopMutation = useStopEmployeeIntel();
  ...
  useEffect(() => {
    ...
    const existingJobId = sessionStorage.getItem(`employee_intel_job_${scanId}`);
    ...
    if (!store.running && !store.complete) {
      startMutation.mutateAsync({ scanId }).then(({ job_id }) => {
        sessionStorage.setItem(`employee_intel_job_${scanId}`, job_id);
  ...
  useEffect(() => {
    ...
    fetchEmployeeIntelReplay(jobId)
  ...
  const handleViewEmployees = () => {
    store.reset();
    sessionStorage.removeItem(`employee_intel_job_${scanId}`);
    onClose();
  };
  ...
  return (
    <Dialog open={open} onClose={handleHide} maxWidth="xs" fullWidth>
      <DialogTitle>Employee Intelligence</DialogTitle>
      <DialogContent>
        {startMutation.isError && (
          <Alert severity="error" sx={{ mb: 2 }}>
            Failed to start gathering. Check that LinkedIn credentials and Hunter.io API keys are configured in Settings.
          </Alert>
        )}
        {store.complete ? (
          <Box sx={{ textAlign: 'center', py: 2 }}>
            ...
            <Typography variant="body2" sx={{ color: tokens.text.secondary }}>
              {store.totalFound} new employee{store.totalFound !== 1 ? 's' : ''} found
            </Typography>
          </Box>
        ) : (
          <Box>
